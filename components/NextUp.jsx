...
export default function NextUp({data, currentVidObj, setCurrentVidObj}) {
    ...
    data.forEach((element) => {
        ...
        next_up.push(
            <>
                <div key={element.url_name} onClick={() => handleInputChange(element.id)} onMouseEnter={() => setIsShown(element.name)} onMouseLeave={() => setIsShown("")} className='flex mt-5 pl-0 p-2 hover:bg-zinc-100'>
                    <img alt={"Thumbnail for "+element.name} className='object-cover rounded w-16 h-16' src={"https://img.youtube.com/vi/"+element.src+"/mqdefault.jpg"} />
